(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))n(r);new MutationObserver(r=>{for(const i of r)if(i.type==="childList")for(const s of i.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&n(s)}).observe(document,{childList:!0,subtree:!0});function o(r){const i={};return r.integrity&&(i.integrity=r.integrity),r.referrerPolicy&&(i.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?i.credentials="include":r.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(r){if(r.ep)return;r.ep=!0;const i=o(r);fetch(r.href,i)}})();const m=[{name:"深邃紫色",gradient:"linear-gradient(135deg, #2d1b69 0%, #11998e 100%)"},{name:"暗夜蓝紫",gradient:"linear-gradient(135deg, #1e3c72 0%, #2a5298 100%)"},{name:"深海蓝绿",gradient:"linear-gradient(135deg, #134e5e 0%, #71b280 100%)"},{name:"墨绿森林",gradient:"linear-gradient(135deg, #0f4c75 0%, #3282b8 100%)"},{name:"暗紫红色",gradient:"linear-gradient(135deg, #360033 0%, #0b8793 100%)"},{name:"深蓝渐变",gradient:"linear-gradient(135deg, #1a2a6c 0%, #b21f1f 50%, #fdbb2d 100%)"},{name:"炭黑蓝色",gradient:"linear-gradient(135deg, #232526 0%, #414345 100%)"},{name:"深紫靛蓝",gradient:"linear-gradient(135deg, #667db6 0%, #0082c8 0%, #0082c8 0%, #667db6 100%)"},{name:"暗绿青色",gradient:"linear-gradient(135deg, #2b5876 0%, #4e4376 100%)"},{name:"深灰蓝色",gradient:"linear-gradient(135deg, #2c3e50 0%, #4a6741 100%)"},{name:"午夜蓝色",gradient:"linear-gradient(135deg, #0c0c0c 0%, #2d5016 50%, #0c0c0c 100%)"},{name:"深橄榄绿",gradient:"linear-gradient(135deg, #1f4037 0%, #99f2c8 100%)"}];let d=0,u=null;function O(){const e=document.createElement("div");e.id="bg-selector",e.style.cssText=`
  ...
  `;const t=e.querySelector("#bg-options");return m.forEach((o,n)=>{const r=document.createElement("div");r.style.cssText=`
    ...
    `,r.addEventListener("click",()=>S(n)),r.addEventListener("mouseenter",()=>{r.style.transform="scale(1.05)",r.style.boxShadow="0 5px 15px rgba(0,0,0,0.3)"}),r.addEventListener("mouseleave",()=>{r.style.transform="scale(1)",r.style.boxShadow="none"}),t.appendChild(r)}),e.querySelector("#close-selector").addEventListener("click",b),e.querySelector("#random-bg").addEventListener("click",()=>{q(),b()}),e.querySelector("#cycle-bg").addEventListener("click",()=>{T(),b()}),document.body.appendChild(e),e}function L(){u||(u=O()),u.style.display="block",$();const e=document.createElement("div");e.id="bg-overlay",e.style.cssText=`
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
  ...
  `,document.body.appendChild(l))}function y(){if(!l||a.detectedImages.length===0)return;const e=a.folder+a.detectedImages[g],t=new Image;t.onload=()=>{l.style.backgroundImage=`url('${e}')`,l.style.opacity=f?a.opacity:0},t.onerror=()=>{console.warn(`照片墙: 无法加载图片 ${e}`),a.detectedImages.length>1&&v()},t.src=e}function v(){a.detectedImages.length!==0&&(g=(g+1)%a.detectedImages.length,y(),k())}function A(){a.detectedImages.length!==0&&(g=(g-1+a.detectedImages.length)%a.detectedImages.length,y(),k())}function z(){if(a.detectedImages.length===0)return;g=Math.floor(Math.random()*a.detectedImages.length),y(),k()}function M(){C(),h=setInterval(v,a.interval)}function C(){h&&(clearInterval(h),h=null)}function W(){if(a.detectedImages.length===0){p("没有找到照片文件，请先添加图片到 public/photos/ 目录");return}f=!f;const e=document.getElementById("photo-btn");f?(F(),y(),M(),X(),e&&(e.textContent="📸 关闭照片墙",e.style.background="rgba(255,255,255,0.4)",e.style.boxShadow="0 0 10px rgba(255,255,255,0.3)"),document.addEventListener("keydown",P),p(`照片墙已启动 (${a.detectedImages.length}张照片) - 空格键切换`)):(l&&(l.style.opacity="0"),C(),Y(),e&&(e.textContent="📸 照片墙",e.style.background="rgba(255,255,255,0.2)",e.style.boxShadow="none"),document.removeEventListener("keydown",P),p("照片墙已关闭"))}function X(){let e=document.getElementById("photo-controls");e||(e=document.createElement("div"),e.id="photo-controls",e.style.cssText=`
      position: fixed;
      bottom: 2rem;
      right: 2rem;
...
<div id="controls-content">
  <!-- 原来的控制内容保持不变 -->
    ...
    `,e.querySelector("#prev-photo").addEventListener("click",A),e.querySelector("#next-photo").addEventListener("click",v),e.querySelector("#random-photo").addEventListener("click",z),e.querySelector("#pause-btn").addEventListener("click",j),e.querySelector("#opacity-btn").addEventListener("click",D),e.querySelector("#minimize-btn").addEventListener("click",V),e.querySelector("#opacity-slider").addEventListener("input",n=>{a.opacity=parseFloat(n.target.value),y(),I()}),e.querySelectorAll("button").forEach(n=>{n.addEventListener("mouseenter",()=>{n.style.background="rgba(255,255,255,0.4)",n.style.transform="scale(1.05)"}),n.addEventListener("mouseleave",()=>{n.style.background="rgba(255,255,255,0.2)",n.style.transform="scale(1)"})}),document.body.appendChild(e)),e.style.display="block",k(),I()}function Y(){const e=document.getElementById("photo-controls");e&&(e.style.display="none")}function k(){const e=document.getElementById("photo-info");if(e&&a.detectedImages.length>0){const t=a.detectedImages[g];e.textContent=`${t} (${g+1}/${a.detectedImages.length})`}}function I(){const e=document.getElementById("opacity-value"),t=document.getElementById("opacity-slider");e&&(e.textContent=`${Math.round(a.opacity*100)}%`),t&&(t.value=a.opacity)}function D(){const e=[.2,.3,.4,.5,.6],o=(e.indexOf(a.opacity)+1)%e.length;a.opacity=e[o],y(),I(),p(`透明度: ${Math.round(a.opacity*100)}%`)}function j(){const e=document.getElementById("pause-btn");h?(C(),e&&(e.textContent="▶️ 继续",e.style.background="rgba(76, 175, 80, 0.3)"),p("照片墙已暂停")):(M(),e&&(e.textContent="⏸️ 暂停",e.style.background="rgba(255,255,255,0.2)"),p("照片墙继续播放"))}function P(e){if(f)switch(e.key){case" ":case"ArrowRight":e.preventDefault(),v();break;case"ArrowLeft":e.preventDefault(),A();break;case"p":case"P":e.preventDefault(),j();break;case"r":case"R":e.preventDefault(),z();break;case"o":case"O":e.preventDefault(),D();break}}function p(e){const t=document.createElement("div");t.textContent=e,t.style.cssText=`
    position: fixed;
    top: 2rem;
    left: 50%;
