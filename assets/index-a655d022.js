...
<div id="controls-content">
  <!-- 原来的控制内容保持不变 -->
    ...
    `,e.querySelector("#prev-photo").addEventListener("click",A),e.querySelector("#next-photo").addEventListener("click",v),e.querySelector("#random-photo").addEventListener("click",z),e.querySelector("#pause-btn").addEventListener("click",j),e.querySelector("#opacity-btn").addEventListener("click",D),e.querySelector("#minimize-btn").addEventListener("click",Z),e.querySelector("#opacity-slider").addEventListener("input",n=>{a.opacity=parseFloat(n.target.value),y(),I()}),e.querySelectorAll("button").forEach(n=>{n.addEventListener("mouseenter",()=>{n.style.background="rgba(255,255,255,0.4)",n.style.transform="scale(1.05)"}),n.addEventListener("mouseleave",()=>{n.style.background="rgba(255,255,255,0.2)",n.style.transform="scale(1)"})}),document.body.appendChild(e)),e.style.display="block",k(),I()}function Y(){const e=document.getElementById("photo-controls");e&&(e.style.display="none")}function k(){const e=document.getElementById("photo-info");if(e&&a.detectedImages.length>0){const t=a.detectedImages[g];e.textContent=`${t} (${g+1}/${a.detectedImages.length})`}}function I(){const e=document.getElementById("opacity-value"),t=document.getElementById("opacity-slider");e&&(e.textContent=`${Math.round(a.opacity*100)}%`),t&&(t.value=a.opacity)}function D(){const e=[.2,.3,.4,.5,.6],o=(e.indexOf(a.opacity)+1)%e.length;a.opacity=e[o],y(),I(),m(`透明度: ${Math.round(a.opacity*100)}%`)}function j(){const e=document.getElementById("pause-btn");h?(C(),e&&(e.textContent="▶️ 继续",e.style.background="rgba(76, 175, 80, 0.3)"),m("照片墙已暂停")):(M(),e&&(e.textContent="⏸️ 暂停",e.style.background="rgba(255,255,255,0.2)"),m("照片墙继续播放"))}function $(e){if(f)switch(e.key){case" ":case"ArrowRight":e.preventDefault(),v();break;case"ArrowLeft":e.preventDefault(),A();break;case"p":case"P":e.preventDefault(),j();break;case"r":case"R":e.preventDefault(),z();break;case"o":case"O":e.preventDefault(),D();break}}function m(e){const t=document.createElement("div");t.textContent=e,t.style.cssText=`
    position: fixed;
    top: 2rem;
    left: 50%;
